...
#Materi
if (selected == 'Materi') :
    st.title('Materi Titrimetri dan Kimia Organik')
...
if (selected == 'Pengolahan Data Titrimetri'):
    ...
    Beratekivalen = st.number_input('Masukkan berat ekivalen/berat molekul dalam sample')
    Molaritas = st.number_input('Masukkan Molaritas/Normalitas(jika tidak digunakan isi dengan 0)')
    tombol1=st.button('hitung normalitas')
    if tombol1:
        nilai_normalitas=bobot/(volume*Beratekivalen)
        st.success(f'nilai normalitas adalah{nilai_normalitas}')
    tombol2=st.button('hitung molaritas')
    if tombol2:
        nilai_Molaritas=bobot/(volume*Beratmolekul)
